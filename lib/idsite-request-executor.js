...
IdSiteRequestExecutor.prototype.handleResponse = function handleResponse (err,response,body,requestorCallback) {
  ...
  var redirectUrl = response.headers['stormpath-sso-redirect-location'];

  if (newToken) {
    parsedToken = newToken.split('Bearer');

    if (parsedToken.length!==2) {
      return requestorCallback(new Error('Authorization header response is not a Bearer token response'));
    }
  ...
  if (redirectUrl) {
    return requestorCallback(null, { redirectUrl: redirectUrl });
  }

  if (!newToken) {
    return requestorCallback(new Error('Authorization header not found'));
  }
