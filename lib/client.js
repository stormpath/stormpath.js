...
function Client (options,readyCallback) {
  ...
  if (!self.jwt) {
    return cb(new Error(strings.errors.JWT_NOT_FOUND));
  }

  jwtSegments = self.jwt.split('.');

  if (jwtSegments.length < 2 || jwtSegments.length > 3) {
    return cb(new Error(strings.errors.NOT_A_JWT));
  }

  try {
    self.jwtPayload = JSON.parse(base64.atob(jwtSegments[1]));
  } catch (e) {
    return cb(new Error(strings.errors.MALFORMED_JWT_CLAIMS));
  }
  ...
  self.requestExecutor = new IdSiteRequestExecutor(self.jwt);
